    ...
    def create_options(self):
      ...
      for struc in self.structure:
        if (self.question.type == 'ITEM_LIST_SINGLE_CHOICE' or self.question.type == 'ITEM_LIST_SINGLE_CHOICE_INTERRUPTIBLE_START') and struc == 'ITEM(Single)':
          ...
          self.options_order, self.options_id = increase_order_id(self.options_order, self.options_id)
        elif self.question.type == 'ITEM_LIST_EXPANDABLE_SINGLE_CHOICE' and struc == 'ITEM(Single)':
          self.options.append(AnswerOptionOption(self, 'CHECKBOX'))
          self.options_order, self.options_id = increase_order_id(self.options_order, self.options_id)
      ...
      if self.question.type == 'OPTION_QUESTION':
          ...
          self.options_order, self.options_id = increase_order_id(self.options_order, self.options_id)
      elif self.question.type == 'ITEM_LIST_MULTI_REF_CUSTOM_AND_NORMAL_EXPANDABLE_NO_LIMIT':
        self.options.append(AnswerOptionOption(self, 'ITEM_LIST_MULTI_REF_CUSTOM_AND_NORMAL_EXPANDABLE_NO_LIMIT'))
        self.options_order, self.options_id = increase_order_id(self.options_order, self.options_id)
      elif self.question.type == 'ITEM_LIST_REF_CUSTOM_SINGLE_CHOICE':
          self.options.append(AnswerOptionOption(self, 'ITEM_LIST_REF_CUSTOM_SINGLE_CHOICE'))
          self.options_order, self.options_id = increase_order_id(self.options_order, self.options_id)
      return self.options
